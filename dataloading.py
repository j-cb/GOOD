...
def getloader_NotMNIST(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.ImageFolder(paths_config.location_dict['NotMNIST']+ '/notMNIST_small', transform=transform) # deleted corrupted files ['A'] = {'RGVtb2NyYXRpY2FCb2xkT2xkc3R5bGUgQm9sZC50dGY=.png'}, ['F'] = {'Q3Jvc3NvdmVyIEJvbGRPYmxpcXVlLnR0Zg==.png'}
    loader = torch.utils.data.DataLoader(
        dset,
        batch_size=batch_size,
        shuffle=train,
...
def getloader_TINY(train, batch_size, augmentation, dataloader_kwargs):
    ...
    tiny_dset = tiny_utils.tinyimages_80mn_loader.TinyImages(transform=transform, exclude_cifar=['H'])# + train*['CEDA11']) #Since it does not make a noticable difference for GOOD, for simplicity we do not use this pre-filtering.
    if train:
